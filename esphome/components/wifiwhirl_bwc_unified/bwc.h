...
public:
    ...
    // ESPHome-friendly hardware setup: bypass filesystem hwcfg.json and use
    // explicit models + pins. This keeps the original MITM architecture
    // (CIO <-> ESP <-> DSP) intact, so the physical display remains usable.
    void setup_hardware(Models cio_model, Models dsp_model, const int pins_in[8]);
