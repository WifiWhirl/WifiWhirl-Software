...
void BWC::setup(void)
{
    Models ciomodel = MIAMI2021;
    Models dspmodel = MIAMI2021;

    // Default pins for the common NodeMCU / PCB2B wiring.
    pins[0] = D1;
    pins[1] = D2;
    pins[2] = D3;
    pins[3] = D4;
    ...
    // If hwcfg.json exists, override defaults.
    _loadHardware(ciomodel, dspmodel, pins);

    setup_hardware(ciomodel, dspmodel, pins);
}

void BWC::setup_hardware(Models cio_model, Models dsp_model, const int pins_in[8])
{
    for (int i = 0; i < 8; i++)
    {
        pins[i] = pins_in[i];
