...
function handlemsg(e) {
  ...
  if (msgobj.CONTENT == "OTHER") {
    ...
    document.getElementById("jetstotals").style.display = msgobj.HASJETS ? "" : "none";
    // godmode available
  ...
  if (msgobj.CONTENT == "TIMES") {
    ...
    document.getElementById("cltimer").innerHTML = (clDateRound == 1 ? 'einem' : clDateRound) + '  Tag' + (clDateRound != 1 ? 'en' : '');
    ...
    document.getElementById("ftimer").innerHTML = (fDateRound == 1 ? 'einem ' : fDateRound) + ' Tag' + (fDateRound != 1 ? 'en' : '');
    ...
    document.getElementById("t2r").innerHTML = s2dhms(msgobj.T2R * 3600) + " (" + (msgobj.RS == 'Ready' ? 'Bereit' : 'Nicht bereit' ) + ")";
  }
}
...
function s2dhms(val) {
  ...
  return (days >= 1 ? days + ' Tag' + (days != 1 ? 'e' : '') + ' ' : '') + hours.toString().pad("0", 2) + ":" + minutes.toString().pad("0", 2) + ":" + seconds.toString().pad("0", 2);
}
